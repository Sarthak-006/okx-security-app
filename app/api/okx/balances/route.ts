...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const method = "GET"
    const requestPath = `/api/v5/dex/aggregator/account/balance?address=${address}&chainId=${chainId}`
    ...
    return NextResponse.json({ error: "Failed to fetch balances from OKX API" }, { status: 500 })
  }
}
