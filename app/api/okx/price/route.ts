...
export async function GET(request: NextRequest) {
  ...
  if (!tokenAddress || !chainId) {
    return NextResponse.json({ error: "TokenAddress and chainId are required" }, { status: 400 })
  }
  ...
  try {
    ...
    const method = "GET"
    const requestPath = `/api/v5/dex/aggregator/market-price?tokenAddress=${tokenAddress}&chainId=${chainId}`
    ...
    return NextResponse.json({ error: "Failed to fetch price from OKX API" }, { status: 500 })
  }
}
